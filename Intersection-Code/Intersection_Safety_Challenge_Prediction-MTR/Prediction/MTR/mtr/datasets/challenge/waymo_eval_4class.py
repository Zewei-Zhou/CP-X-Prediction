...
def waymo_evaluation(pred_dicts, top_k=-1, eval_second=8, num_modes_for_eval=6):
    ...
    for i, m in enumerate(['minADE', 'minFDE', 'MissRate', 'OverlapRate', 'mAP']):
        avg_results.update({
            f'{m} - VEHICLE': [0.0, 0], f'{m} - PEDESTRIAN': [0.0, 0], f'{m} - CYCLIST': [0.0, 0], f'{m} - DUMMY': [0.0, 0]
        })
    ...
    name_to_row = {'VEHICLE': 1, 'PEDESTRIAN': 2, 'CYCLIST': 3, 'DUMMY': 4, 'Avg': 5}
    ...
    for cur_metric_name in ['minADE', 'minFDE', 'MissRate', 'mAP']:
        ...
        for cur_name in ['VEHICLE', 'PEDESTRIAN', 'CYCLIST', 'DUMMY']:
