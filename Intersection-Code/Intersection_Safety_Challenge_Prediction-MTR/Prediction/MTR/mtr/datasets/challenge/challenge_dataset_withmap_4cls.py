...
class ChallengeDatasetWithMap_4Cls(DatasetTemplate):
    ...
    def filter_info_by_object_type(self, infos, valid_object_types=None):
        ...
        for cur_info in infos:
            ...
            assert len(cur_info['tracks_to_predict'].keys()) == 4, f"{cur_info['tracks_to_predict'].keys()}"
    ...
    def weighted_ade_evaluation(self, pred_dicts, num_modes_for_eval=6):
        avg_weighted_ade = []
        ...
        for i in range(len(pred_dicts)):
            for j in range(len(pred_dicts[i])):
                pred_trajs = pred_dicts[i][j]['pred_trajs']
                gt_trajs = pred_dicts[i][j]['gt_trajs'][11:,:2]
                pred_scores = pred_dicts[i][j]['pred_scores']
                ...
                if pred_dicts[i][j]['object_type'] == 'TYPE_VEHICLE':
                    avg_weighted_ade_vehicle += [weighted_ade]
                elif pred_dicts[i][j]['object_type'] == 'TYPE_PEDESTRIAN':
                    avg_weighted_ade_pedestrian += [weighted_ade]
                elif pred_dicts[i][j]['object_type'] == 'TYPE_CYCLIST':
                    avg_weighted_ade_cyclist += [weighted_ade]
                elif pred_dicts[i][j]['object_type'] == 'TYPE_DUMMY':
        ...
        weighted_ade_metrics = {
            "avg_weighted_ade": avg_weighted_ade,
            "avg_weighted_ade_vehicle": avg_weighted_ade_vehicle,
            "avg_weighted_ade_pedestrian": avg_weighted_ade_pedestrian,
            "avg_weighted_ade_cyclist": avg_weighted_ade_cyclist,
            "avg_weighted_ade_dummy": avg_weighted_ade_dummy
        }
        return weighted_ade_metrics



    def evaluation(self, pred_dicts, output_path=None, eval_method='waymo', **kwargs):
        eval_method = 'weighted_ade'
        if eval_method == 'waymo':
            from .waymo_eval_4class import waymo_evaluation
            try:
        ...
        elif eval_method == 'weighted_ade':
            try:
                num_modes_for_eval = pred_dicts[0][0]['pred_trajs'].shape[0]
            except:
                num_modes_for_eval = 6
            metric_results = self.weighted_ade_evaluation(pred_dicts, num_modes_for_eval)
