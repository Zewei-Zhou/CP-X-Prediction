...
def rotate_points_along_z_single(points, angle):
    """
    Args:
        points: (2,), a single point in 2D
        angle: (1,), angle along z-axis, angle increases x ==> y
    Returns:
        Rotated point
    """
...
def process_waymo_data_with_scenario_proto(data_file, output_path=None):
    ...
    for cnt, data in enumerate(dataset):
        ...
        track_to_predict_origin = {
            'track_index': [cur_pred.track_index for cur_pred in scenario.tracks_to_predict],
            'difficulty': [cur_pred.difficulty for cur_pred in scenario.tracks_to_predict]
        }  # for training: suggestion of objects to train on, for val/test: need to be predicted

        track_infos = decode_tracks_from_proto(scenario.tracks)
        # info['tracks_to_predict']['object_type'] = [track_infos['object_type'][cur_idx] for cur_idx in track_to_predict_origin['track_index']]
        track_to_predict_origin['object_type'] = [track_infos['object_type'][cur_idx] for cur_idx in track_to_predict_origin['track_index']]

        # decode map related data
        map_infos = decode_map_features_from_proto(scenario.map_features)
        dynamic_map_infos = decode_dynamic_map_states_from_proto(scenario.dynamic_map_states)

        # filter scenario by map_infos, if any lane is freeway, filter out the whole scenario
        lanes = map_infos['lane']
        for lane in lanes:
            if lane['type'] == 'TYPE_FREEWAY':
                continue
        
        # filter info['tracks_to_predict'] by last valid point's distance
        track_to_predict_filtered = {}
        track_to_predict_filtered['track_index'] = []
        track_to_predict_filtered['object_type'] = []
        
        for cur_predict_track_index in track_to_predict_origin['track_index']:
            object_type = track_infos['object_type'][cur_predict_track_index]

            traj = track_infos['trajs'][cur_predict_track_index]
            last_valid_idx = np.where(traj[:, -1] > 0)[0][-1]
            last_valid_point = traj[last_valid_idx, :2]
            center_heading_current = traj[info['current_time_index'], 6]
            center_xyz_current = traj[info['current_time_index'], :2]
            last_valid_point_transformed = last_valid_point - center_xyz_current
            last_valid_point_transformed = rotate_points_along_z_single(last_valid_point_transformed, -center_heading_current)

            if object_type == 'TYPE_VEHICLE':
                # x in [-20, 70] y in [-30, 50]
                if last_valid_point_transformed[0] >= -20 and last_valid_point_transformed[0] <= 70 and last_valid_point_transformed[1] >= -30 and last_valid_point_transformed[1] <= 50:
                    track_to_predict_filtered['track_index'].append(cur_predict_track_index)
                    track_to_predict_filtered['object_type'].append(object_type)

            elif object_type == 'TYPE_PEDESTRIAN':
                # x in [-10, 17.5] y in [-15, 15]
                if last_valid_point_transformed[0] >= -10 and last_valid_point_transformed[0] <= 17.5 and last_valid_point_transformed[1] >= -15 and last_valid_point_transformed[1] <= 15:
                    track_to_predict_filtered['track_index'].append(cur_predict_track_index) 
                    track_to_predict_filtered['object_type'].append(object_type)

            elif object_type == 'TYPE_CYCLIST':
                # x in [-12.5,25] y  in [-17.5,22.5]
                if last_valid_point_transformed[0] >= -12.5 and last_valid_point_transformed[0] <= 25 and last_valid_point_transformed[1] >= -17.5 and last_valid_point_transformed[1] <= 22.5:
                    track_to_predict_filtered['track_index'].append(cur_predict_track_index)
                    track_to_predict_filtered['object_type'].append(object_type)

        if len(track_to_predict_filtered['track_index']) == 0:
            continue
        
        info['tracks_to_predict'] = track_to_predict_filtered
...
def get_infos_from_protos(data_path, output_path=None, num_workers=8):
    ...
    data_infos = [infos for infos in data_infos if len(infos) != 0]
