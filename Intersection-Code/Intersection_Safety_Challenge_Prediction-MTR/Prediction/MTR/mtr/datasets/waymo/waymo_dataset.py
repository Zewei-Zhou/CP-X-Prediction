...
class WaymoDataset(DatasetTemplate):
    ...
    # used to get map data by scenario id for plotting the map against the predicted and historical trajectories
    def get_map_data_by_id(self, scenario_id):
        """
        Extracts the raw map polylines and info for a given scenario_id.
        """
        # 1. Find the path to the raw scene file
        scene_file_path = self.data_path / f'sample_{scenario_id}.pkl'
        
        if not scene_file_path.exists():
            self.logger.error(f"Scene file not found: {scene_file_path}")
            return None

        # 2. Load the full scene info from the pickle file
        with open(scene_file_path, 'rb') as f:
            info = pickle.load(f)

        # 3. Extract the map information dictionary
        # This dictionary contains 'all_polylines', 'road_polylines', etc.
        if 'map_infos' in info:
            raw_map_data = info['map_infos']
        else:
            self.logger.warning(f"Map data not found for scenario {scenario_id}")
            return None
