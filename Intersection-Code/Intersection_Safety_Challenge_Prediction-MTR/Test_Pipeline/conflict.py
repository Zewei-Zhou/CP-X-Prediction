...
def check_conflict(ru1_traj, ru2_traj, ru1_size, ru2_size, ru1_subclass, ru2_subclass):
    ...
    conflict_point, conflict_step = get_conflict_point(ru1_traj, overlap_area)

    # allow more timesteps to check for conflict
    for t in range(min(conflict_step, LOOK_AHEAD)):
...
def calculate_conflict(prediction_results, objects):
        ...
        for j in prediction_results.keys():
            ...
            for ru1_traj, ru1_score in zip(ru1_possible_trajs, ru1_traj_scores):
                for ru2_traj, ru2_score in zip(ru2_possible_trajs, ru2_traj_scores):
                    ...
                    # ignore conflict between two vehicles
                    if 'Vehicle' in ru1_subclass and 'Vehicle' in ru2_subclass:
                        continue
                    ...
                    # if conflict, add to results
                    if conflict:
                        print(f'Conflict between {i} and {j} at t={ttc}')
    ...
    if len(results) == 0:
        ...
        # if vehicles are involved, get the minimum ttc only for vehicles
        if 'Passenger_Vehicle' in [r[1] for r in results] or 'Passenger_Vehicle' in [r[2] for r in results]:
            ttc = np.min([r[0] for r in results if 'Passenger_Vehicle' in r[1] or 'Passenger_Vehicle' in r[2]])
            idx = np.argmin([r[0] for r in results if 'Passenger_Vehicle' in r[1] or 'Passenger_Vehicle' in r[2]])
            ru1_subclass = results[idx][1]
            ru2_subclass = results[idx][2]
        
        # if no vehicles are involved, get the minimum ttc
        else:
