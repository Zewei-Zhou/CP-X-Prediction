...
class MTR(pl.LightningModule):
    def __init__(
        ...
        super().__init__()
        self.cfg = cfg
        self.encoder = Encoder(cfg['history_len'], cfg['encoder_layers'])
        self.predictor = Predictor(cfg['decoder_layers'], cfg['future_len'], cfg['modality'])
    ...
    def forward_and_get_loss(self, batch, prefix=''):
        ...
        for k in range(level_K):
            ...
            pred_loss = traj_loss_mean + 0.1 * weighted_ade_loss + 0.05 * score_loss_mean 
            total_loss += pred_loss
        
        # Calculate metrics
        trajs = predictor_outputs[f"layer_{self.cfg['decoder_layers']-1}_trajs"]
        scores = predictor_outputs[f"layer_{self.cfg['decoder_layers']-1}_scores"].softmax(dim=-1)
    ...
    def calculate_metrics_predict(self,
        ...
        weights = scores / torch.sum(scores, dim=-1, keepdim=True)
...
class AgentEncoder(nn.Module):
    def __init__(self, hist_len=30, d_model=256, d_ffn=1024, n_heads=8, dropout=0.1):
        super().__init__()
        self.hist_len = hist_len
        self.type_embed = nn.Embedding(13, d_model, padding_idx=0)
        self.time_embed = nn.Embedding(self.hist_len, d_model)
        ...
        self.register_buffer('time', torch.arange(self.hist_len).long())
    ...
    def forward(self, object_trajs, valid_mask):
        ...
        return encoded_objects, type_embed


class Encoder(nn.Module):
    def __init__(self, hist_len=30, layers=6):
        super().__init__()
        self.agent_encoder = AgentEncoder(hist_len)
    ...
    def forward(self, inputs):
        ...
        mask[:, :, 0] = False
        encoded_agents, encoded_agents_type = self.agent_encoder(agents, mask)
        ...
        encoder_outputs['agents_type'] = encoded_agents_type
...
class Predictor(nn.Module):
    def __init__(self, layers=6, future_len=50, modality=3):
        super().__init__()     
        self._agents_len = 5   
        self._future_len = future_len
        self._num_modalities = modality
