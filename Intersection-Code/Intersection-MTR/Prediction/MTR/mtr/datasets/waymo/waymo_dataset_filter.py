...
class WaymoDataset_filter(DatasetTemplate):
    ...
    def filter_info_by_object_type(self, infos, valid_object_types=None):
        ...
        for cur_info in infos:
            ...
            assert len(cur_info['tracks_to_predict'].keys()) == 2, f"{len(cur_info['tracks_to_predict'].keys())}"
            assert len(cur_info['tracks_to_predict'].keys()) == 2, f"{cur_info['tracks_to_predict'].keys()}"
