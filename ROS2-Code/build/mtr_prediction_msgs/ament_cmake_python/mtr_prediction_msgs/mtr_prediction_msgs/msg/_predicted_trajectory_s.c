...
bool mtr_prediction_msgs__msg__predicted_trajectory__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("mtr_prediction_msgs.msg._predicted_trajectory.PredictedTrajectory", full_classname_dest, 65) == 0);
  }
  mtr_prediction_msgs__msg__PredictedTrajectory * ros_message = _ros_message;
  {  // confidence
  ...
  {  // velocities
    ...
    if (!geometry_msgs__msg__TwistStamped__Sequence__init(&(ros_message->velocities), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create geometry_msgs__msg__TwistStamped__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    geometry_msgs__msg__TwistStamped * dest = ros_message->velocities.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!geometry_msgs__msg__twist_stamped__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of PredictedTrajectory */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("mtr_prediction_msgs.msg._predicted_trajectory");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "PredictedTrajectory");
  ...
  {  // velocities
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = geometry_msgs__msg__twist_stamped__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
