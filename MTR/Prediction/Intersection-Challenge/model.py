...
class MTR(pl.LightningModule):
    ...
    def check_input_nan(self, batch, batch_idx):
        if torch.isnan(batch['hist_trajs']).any() or torch.isnan(batch['maps']).any():
            print(f"⚠️ Skipping Batch {batch_idx}: Input contains NaN.")
            return None
    
    def check_huge_coordinates(self, batch, batch_idx):
        # --- CHECK 2: Do huge coordinates exist? (Optional but recommended) ---
        # If your data isn't normalized, coordinates > 1000 will likely crash the model later
        if torch.max(torch.abs(batch['hist_trajs'][..., :2])) > 1000:
             print(f"⚠️ Skipping Batch {batch_idx}: Huge coordinates detected (>1000).")
             return None
         
    def check_nan_loss(self, batch, loss, log_dict, batch_idx):
        # Check for NaN/Inf
        if torch.isnan(loss) or torch.isinf(loss):
            print(f"⚠️ SKIPPING BATCH {batch_idx}: Loss is {loss.item()}")
            print(f"\n⚠️ NaN/Inf detected at step {self.global_step}")
            ...
            for k, v in batch.items():
                if isinstance(v, torch.Tensor):
                    ...
                    if nan_count > 0 or inf_count > 0:
                        print(f"  {k}: {nan_count} NaNs, {inf_count} Infs")
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.check_input_nan(batch, batch_idx)
        # check if coordinates are not normalized and are too large, thus casuing large error values
        self.check_huge_coordinates(batch, batch_idx)
        
        loss, log_dict = self.forward_and_get_loss(batch, prefix='train/')
        # check if loss contains nan
        self.check_nan_loss(batch, loss, log_dict, batch_idx)
    ...
    def calculate_metrics_predict(self,
        ...
        weighted_ADE = weighted_mse.sum() / (gt_mask.sum() + 1e-4)
        weighted_FDE = weighted_mse[..., -1].sum() / (gt_mask[..., -1].sum() + 1e-4)
...
class AgentEncoder(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, n_heads=8, dropout=0.1):
        super().__init__()
        self.type_embed = nn.Embedding(22, d_model, padding_idx=0) # 22 object types
...
class MapEncoder(nn.Module):
    def __init__(self, d_model=256):
        ...
        #   5 different TYPE categories
        #   (0=pad, 1=lane, 2=line, 3=cross, 4=other)
        self.type_embed = nn.Embedding(5, d_model, padding_idx=0) # 4 map polyline types
